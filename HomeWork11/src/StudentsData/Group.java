...
public class Group implements Voencom {
	...
	public Table studentsToTable() {
		Table tab = new Table(students.size() + 1, 5, "border='2' cellpadding='5' ", this.name);
		tab.fillCell(0, 0, "Students name");
		tab.fillCell(0, 1, "Sex");
		tab.fillCell(0, 2, "Age");
		tab.fillCell(0, 3, "Grade point average");
		tab.fillCell(0, 4, "Record Book");
		for (int i = 0; i < students.size(); i++) {
			tab.fillCell(i + 1, 0, students.get(i).getName());
			if(students.get(i).getSex() == SexList.MALE) {
				tab.fillCell(i + 1, 1, "Male");
			} else {
				tab.fillCell(i + 1, 1, "Female");
			}
			tab.fillCell(i + 1, 2, "" + students.get(i).getAge());
			tab.fillCell(i + 1, 3, "" + students.get(i).getGradePointAverage());
			tab.fillCell(i + 1, 4, "" + students.get(i).getRecordBookNum());
		}
		return tab;
	}
}